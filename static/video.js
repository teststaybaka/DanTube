...
function widescreen_change() {
	if ($('#wide-screen').hasClass('on')) {
		...
		player_width = 640;
		player_height = 446;
		player_inner_width = 640;
		player_inner_height = 360;
...
dt.onPlayerReady = function(event) {
	...
	$('#advanced-danmaku-form').submit(function(evt) {
		...
		$.ajax({
			type: "POST",
			url: '/video/advanced_danmaku/'+video_id+'/'+clip_id,
			data: data,
			success: function(result) {
				if(!result.error) {
					$('#advanced-danmaku-content').val('');
					dt.pop_ajax_message('Danmaku sent!', 'success');
					result.timestamp = result.timestamp + 0.05;
					result.blocked = false;
					var listNode = document.getElementById("danmaku-list");
					danmaku_pool_list.push(result);
					listNode.appendChild(generate_danmaku_pool_entry(danmaku_pool_list.length-1));
	...
	$('#subtitles-danmaku-form').submit(function(evt) {
		...
		$.ajax({
			type: "POST",
			url: '/video/subtitles_danmaku/'+video_id+'/'+clip_id,
			data: $(this).serialize(),
			success: function(result) {
				if(!result.error) {
	...
	$('#code-danmaku-form').submit(function() {
		...
		$.ajax({
			type: "POST",
			url: '/video/code_danmaku/'+video_id+'/'+clip_id,
			data: data,
			success: function(result) {
				if(!result.error) {
